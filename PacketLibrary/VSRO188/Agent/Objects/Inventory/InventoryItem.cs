...
public class InventoryItem
{
    ...
    {
        get { return _refObjItem ??= Cache.GetRefObjItemAsync((int)ItemId).Result; }
    }
    ...
    public static InventoryItem FromPacket(Packet packet, byte destinationSlot = 0xFE)
    {
        ...
        {
            packet.TryRead(out item.State);
            item.Amount = 1;

            if (item.State != InventoryItemState.Inactive)
            {
                packet.TryRead(out item.Cos.Id)
        ...
        {
            packet.TryRead(out item.Amount); // Quantity
        }
