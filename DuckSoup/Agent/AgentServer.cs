...
public class AgentServer : AsyncServer
{
    public AgentServer(Service service) : base(service)
    {
        ...
        PacketHandler.RegisterClientHandler(0x7060, async (packet, session) =>
        {
            Global.Logger.Info("CLIENT_AGENT_PARTY_CREATE_REQUEST client start" + session.SessionData.Charname);
            await new CLIENT_AGENT_PARTY_CREATE_REQUEST().Read(packet);
            Global.Logger.Info("CLIENT_AGENT_PARTY_CREATE_REQUEST client stop" + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterClientHandler(0x7061, async (packet, session) =>
        {
            Global.Logger.Info("CLIENT_AGENT_PARTY_LEAVE client start" + session.SessionData.Charname);
            await new CLIENT_AGENT_PARTY_LEAVE().Read(packet);
            Global.Logger.Info("CLIENT_AGENT_PARTY_LEAVE client stop" + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterClientHandler(0x7062, async (packet, session) =>
        {
            Global.Logger.Info("CLIENT_AGENT_PARTY_INVITE_REQUEST client start" + session.SessionData.Charname);
            await new CLIENT_AGENT_PARTY_INVITE_REQUEST().Read(packet);
            Global.Logger.Info("CLIENT_AGENT_PARTY_INVITE_REQUEST client stop" + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterClientHandler(0x7063, async (packet, session) =>
        {
            Global.Logger.Info("CLIENT_AGENT_PARTY_KICK_REQUEST client start" + session.SessionData.Charname);
            await new CLIENT_AGENT_PARTY_KICK_REQUEST().Read(packet);
            Global.Logger.Info("CLIENT_AGENT_PARTY_KICK_REQUEST client stop" + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        // PacketHandler.RegisterClientHandler(None, async (packet, session) =>
        // {
        //     Global.Logger.Info("AGENT_PARTY_UPDATE client start");
        //
        //     Global.Logger.Info("AGENT_PARTY_UPDATE client stop");
        //     return new PacketResult(PacketResultType.Nothing);
        // });
        // PacketHandler.RegisterClientHandler(None, async (packet, session) =>
        // {
        //     Global.Logger.Info("AGENT_PARTY_CREATED client start");
        //
        //     Global.Logger.Info("AGENT_PARTY_CREATED client stop");
        //     return new PacketResult(PacketResultType.Nothing);
        // });
        // PacketHandler.RegisterClientHandler(None, async (packet, session) =>
        // {
        //     Global.Logger.Info("AGENT_PARTY_CREATED_FROM_MATCHING client start");
        //
        //     Global.Logger.Info("AGENT_PARTY_CREATED_FROM_MATCHING client stop");
        //     return new PacketResult(PacketResultType.Nothing);
        // });
        // PacketHandler.RegisterClientHandler(None, async (packet, session) =>
        // {
        //     Global.Logger.Info("sro_client.OnJoinPartyAck client start");
        //
        //     Global.Logger.Info("sro_client.OnJoinPartyAck client stop");
        //     return new PacketResult(PacketResultType.Nothing);
        // });
        // PacketHandler.RegisterClientHandler(None, async (packet, session) =>
        // {
        //     Global.Logger.Info("AGENT_PARTY_DISTRIBUTION client start");
        //
        //     Global.Logger.Info("AGENT_PARTY_DISTRIBUTION client stop");
        //     return new PacketResult(PacketResultType.Nothing);
        // });
        PacketHandler.RegisterClientHandler(0x7069, async (packet, session) =>
        {
            Global.Logger.Info("CLIENT_AGENT_PARTY_MATCHING_FORM_REQUEST client start - " + session.SessionData.Charname);
            await new CLIENT_AGENT_PARTY_MATCHING_FORM_REQUEST().Read(packet);
            Global.Logger.Info("CLIENT_AGENT_PARTY_MATCHING_FORM_REQUEST client stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterClientHandler(0x706A, async (packet, session) =>
        {
            Global.Logger.Info("CLIENT_AGENT_PARTY_MATCHING_CHANGE_REQUEST client start - " + session.SessionData.Charname);
            await new CLIENT_AGENT_PARTY_MATCHING_CHANGE_REQUEST().Read(packet);
            Global.Logger.Info("CLIENT_AGENT_PARTY_MATCHING_CHANGE_REQUEST client stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterClientHandler(0x706B, async (packet, session) =>
        {
            Global.Logger.Info("CLIENT_AGENT_PARTY_MATCHING_DELETE_REQUEST client start - " + session.SessionData.Charname);
            await new CLIENT_AGENT_PARTY_MATCHING_DELETE_REQUEST().Read(packet);
            Global.Logger.Info("CLIENT_AGENT_PARTY_MATCHING_DELETE_REQUEST client stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterClientHandler(0x706C, async (packet, session) =>
        {
            Global.Logger.Info("CLIENT_AGENT_PARTY_MATCHING_LIST_REQUEST client start - " + session.SessionData.Charname);
            await new CLIENT_AGENT_PARTY_MATCHING_LIST_REQUEST().Read(packet);
            Global.Logger.Info("CLIENT_AGENT_PARTY_MATCHING_LIST_REQUEST client stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterClientHandler(0x706D, async (packet, session) =>
        {
            Global.Logger.Info("CLIENT_AGENT_PARTY_MATCHING_JOIN_REQUEST client start - " + session.SessionData.Charname);
            await new CLIENT_AGENT_PARTY_MATCHING_JOIN_REQUEST().Read(packet);
            Global.Logger.Info("CLIENT_AGENT_PARTY_MATCHING_JOIN_REQUEST client stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterClientHandler(0x306E, async (packet, session) =>
        {
            Global.Logger.Info("CLIENT_JOIN_FORMED_RESPONSE client start - " + session.SessionData.Charname);
            await new CLIENT_JOIN_FORMED_RESPONSE().Read(packet);
            Global.Logger.Info("CLIENT_JOIN_FORMED_RESPONSE client stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        
        PacketHandler.RegisterModuleHandler(0xB060, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_CREATE_RESPONSE server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_CREATE_RESPONSE().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_CREATE_RESPONSE server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        // DEPRICATED
        // PacketHandler.RegisterModuleHandler(0xB061, async (packet, session) =>
        // {
        //     Global.Logger.Info("AGENT_PARTY_LEAVE server start - TEST -" + session.SessionData.Charname);
        //     // await new SERVER_AGENT_PARTY_CREATE_RESPONSE().Read(packet);
        //     Global.Logger.Info("AGENT_PARTY_LEAVE server stop - TEST -" + session.SessionData.Charname);
        //     return new PacketResult(PacketResultType.Nothing);
        // });
        PacketHandler.RegisterModuleHandler(0xB062, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_INVITE_RESPONSE server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_INVITE_RESPONSE().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_INVITE_RESPONSE server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        // DEPRICATED
        // PacketHandler.RegisterModuleHandler(0xB063, async (packet, session) =>
        // {
        //     Global.Logger.Info("AGENT_PARTY_KICK server start - " + session.SessionData.Charname);
        //     await new SERVER_AGENT_PARTY_INVITE_RESPONSE().Read(packet);
        //     Global.Logger.Info("AGENT_PARTY_KICK server stop - " + session.SessionData.Charname);
        //     return new PacketResult(PacketResultType.Nothing);
        // });
        PacketHandler.RegisterModuleHandler(0x3864, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_UPDATE server start - " + session.SessionData.Charname + " - ");
            var response = new SERVER_AGENT_PARTY_UPDATE();
            await response.Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_UPDATE server stop - " + session.SessionData.Charname + " - " + response.PartyUpdateType);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterModuleHandler(0x3865, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_CREATED server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_CREATED().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_CREATED server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterModuleHandler(0x3065, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_CREATE_FROM_MATCHING server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_CREATE_FROM_MATCHING().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_CREATE_FROM_MATCHING server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterModuleHandler(0xB067, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_JOIN_RESPONSE server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_JOIN_RESPONSE().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_JOIN_RESPONSE server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterModuleHandler(0x3068, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_DISTRIBUTION server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_DISTRIBUTION().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_DISTRIBUTION server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterModuleHandler(0xB069, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_FORM_RESPONSE server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_MATCHING_FORM_RESPONSE().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_FORM_RESPONSE server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterModuleHandler(0xB06A, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_CHANGE_RESPONSE server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_MATCHING_CHANGE_RESPONSE().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_CHANGE_RESPONSE server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterModuleHandler(0xB06B, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_DELETE_RESPONSE server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_MATCHING_DELETE_RESPONSE().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_DELETE_RESPONSE server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterModuleHandler(0xB06C, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_LIST_RESPONSE server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_MATCHING_LIST_RESPONSE().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_LIST_RESPONSE server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterModuleHandler(0xB06D, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_JOIN_RESPONSE server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_MATCHING_JOIN_RESPONSE().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_JOIN_RESPONSE server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
        PacketHandler.RegisterModuleHandler(0x706D, async (packet, session) =>
        {
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_PLAYER_JOIN_REQUEST server start - " + session.SessionData.Charname);
            await new SERVER_AGENT_PARTY_MATCHING_PLAYER_JOIN_REQUEST().Read(packet);
            Global.Logger.Info("SERVER_AGENT_PARTY_MATCHING_PLAYER_JOIN_REQUEST server stop - " + session.SessionData.Charname);
            return new PacketResult(PacketResultType.Nothing);
        });
    }
