...
public class AuthChangePasswordCommand : Command
{
    ...
    public override void Execute(string[]? args)
    {
        ...
        {
            Log.Information("User's {0}[{1}] password was successfully changed and invalidated.", user.username,
                user.userId);
            return;
        }

        Log.Error("There was a error chaing password on the user {0}", username);
    }
}