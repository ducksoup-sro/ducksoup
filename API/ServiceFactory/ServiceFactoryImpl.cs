...
public class ServiceFactoryImpl : IServiceFactory
{
    ...
    public T Register<T>(Type service, object provider)
    {
        if (Providers == null)
        {
            throw new DisposedException(nameof(ServiceFactoryImpl));
        }
    ...
    public T Load<T>(Type service)
    {
        ...
        if (!Providers.ContainsKey(service))
        {
            throw new ServiceNotRegisteredException(service.Name);
        }
    ...
    public Collection<Type> GetKnownServices()
    {
        ...
        foreach (var (key, _) in Providers)
        {
            result.Add(key);
        }
    ...
    public void Dispose()
    {
        ...
        foreach (var registeredServiceProvider in Providers)
        {
